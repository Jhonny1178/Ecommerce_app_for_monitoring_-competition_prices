...
class JmbdesingSpider(scrapy.Spider):
    ...
    def get_ajax_price(self, response):
        ...
        if color is None:
            color = None
        current_match_normal = re.search(r'class="product-price--current"[\s\S]*?>[\s\n]*([\d\s\xa0]+,[\d]{2})', price_html)
        regular_match_special = re.search(r'class="product-price--regular"[\s\S]*?>[\s\n]*([\d\s\xa0]+,[\d]{2})', price_html)
        if current_match_normal:
            price_normal = current_match_normal.group(1)
        else:
            price_normal = None
        if regular_match_special:
            price_normal = regular_match_special.group(1)
            price_special = current_match_normal.group(1)
        else:
            price_special = None
        ...
        product_data['price_normal'] = price_normal
