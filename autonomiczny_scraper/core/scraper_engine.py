...
def extract_after_keyword(text, keywords):
    for keyword in keywords:
        # Zmieniony Regex: WYMUSZAMY dwukropek lub myślnik i łapiemy tylko do najbliższej kropki lub przecinka!
...
def extract_data_using_map(response, css_map):
    ...
    if all_dimensions_data:
        ...
        for combination in itertools.product(*all_dimensions_data):
            ...
            # Oznaczamy parametry dla pająka (żeby wiedział co i jak wywołać)
            variants.append({
                ...
                'brand': brand,
                'old_price': old_price,
                'special_price': special_price,  # Ta cena zostanie ew. nadpisana przez AJAX
                ...
                # Zamiast pojedynczego group/value przekazujemy komplet do AJAX-a
                '_ajax_query_params': query_params,
                '_clean_url': clean_url
            })

    # Jeśli nie znaleziono wariantów (zwykły produkt pojedynczy)
    if not variants:
        variants.append({
